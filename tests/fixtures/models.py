...
class DCMotorModel:
    def __init__(self):
        self.static_propeller = StaticPropeller(
            0.09, 0.04, 8*25.E-3, name="static_propeller")
        self.dcmotor = DCMotor(789.E-6, 43.3E-3, 1.9E-3,
                               5.284E-6, name="dcmotor")

        self.engine = NonLeafBlock(name="engine",
                                   children=[self.dcmotor,
                                             self.static_propeller],
                                   num_inputs=2,
                                   num_outputs=2)
        ...
        self.leaf_blocks = set(
            [self.dcmotor, self.static_propeller, self.voltage, self.density])


class ExponentialDecay(LTISystem):
    def __init__(self, T, **kwargs):
        LTISystem.__init__(
            self, A=[[-1/T]], B=np.zeros((1, 0)), C=[[1]], D=np.zeros((1, 0)), **kwargs)
...
def decay_model_no_state():
    exp_decay_no_state = ExponentialDecayNoState(
        T=1., x0=10.0, name="decay_no_state")
    exp_decay_state = ExponentialDecay(
        T=1., initial_condition=[10.0], name="decay_state")
